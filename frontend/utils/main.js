const { resolve_gen, base58_gen, compress_gen, sign_gen, address_gen, public_gen, payload_gen } = require("./crypto_mainet");
...
const root_hash = compress_gen(`did:compress:${address[0]}-${address[1]}-${address[2]}`)
...
const did1 = `did:mainnet:${address[0]}`
const did2 = `did:ropsten:${address[1]}`
const did3 = `did:mainnet:${address[2]}`

console.log(root_hash)
// console.log(`did:resolve:${did1}.${did2}.${did3}-${sign1}.${sign2}.${sign3}`)

console.log(payload_gen(`did:resolve:${did1}.${did2}.${did3}-${sign1}.${sign2}.${sign3}`))
