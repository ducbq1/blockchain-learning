...
module.exports = {
  ...
  compress_gen: (input) =>
    compressMessage(input.split(":")[2].split("-")).toString("hex"),
  // compress_gen: (input) => {
...
const sign = getSign(
  "afa3cc1d1a7621d2624ea3ebbd530d87556b29c39402cdb1ce8874aaeeba6142",
  privateKey[2]
);
...
function base58(address_hex) {
  ...
  while (address_int > 0) {
    let digit = address_int % BigInt(58);
    base58string = alphabet[Number(digit)] + base58string;
    address_int = address_int / BigInt(58);
  }
...
function resolvePayload({ data: array_did, signature: array_signature }) {
  return {
    timestamp: Date.now(),
    count: array_did.length,
    data: array_did,
...
function getSign(message, _privateKey) {
  ...
  return (
    "0x" +
    Buffer.from(sign.signature).toString("hex") +
    sign.recid.toString().padStart(2, "0")
  );
}
...
function getAddress(_privateKey, chain, type = "p2pkh") {
  ...
  if (chain == "binance") {
    ...
    const hex = Buffer.from(getPublicKeyCompressed(_privateKey)).toString(
      "hex"
    );
    const binary = hex
      .split("")
