...
contract MultiSignature is MerkleTree, Context, ERC20 {
    string public constant VERSION = "1.0.0";
    string public constant NAME = "Sign Payload";
    ...
    bytes32 constant TXN_TYPEHASH =
        keccak256(
            "Transaction(address destination, uint256 value, bytes data, bool executed)"
        );

    bytes32 constant VALIDATOR_TYPEHASH =
        keccak256("Validator(string name, address wallet)");

    bytes32 constant AUTHORITY_TYPEHASH =
        keccak256("Authority(string name, string contents)");
    ...
    constructor(
        ...
        ERC20("MultiSignature", "MS", 18)
        validRequirement(_owners.length, _threshold)
    {
        _mint(_msgSender(), 10**7);
            ...
            Cryptography.recover(
                keccak256(
                    abi.encodePacked(
                        "\x19\x01",
                        domainSeparator(),
                        hash(Validator({name: "ADD_OWNER", wallet: owner}))
                    )
                ),
                signatures
            ) == owner
            ...
            Cryptography.recover(
                keccak256(
                    abi.encodePacked(
                        ...
                        domainSeparator(),
                        hash(
                            Validator({name: "REPLACE_OWNER", wallet: newOwner})
                        )
                    )
                ),
                signatures
    ...
    function confirmTransaction(uint256 nonce)
        ...
        notConfirmed(nonce, _msgSender())
    {
        confirmations[nonce][_msgSender()] = true;
        emit Confirmation(_msgSender(), nonce);
        executeTransaction(nonce);
    }
    ...
    function revokeConfirmation(uint256 nonce)
        public
        ownerExists(_msgSender())
        confirmed(nonce, _msgSender())
        notExecuted(nonce)
    {
        confirmations[nonce][_msgSender()] = false;
        emit Revocation(_msgSender(), nonce);
    }
    ...
    function hash(Validator memory validator) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    VALIDATOR_TYPEHASH,
                    keccak256(bytes(validator.name)),
                    validator.wallet
                )
            );
    }
        ...
        return
            keccak256(
                abi.encode(
                    ...
                    keccak256(bytes(authority.contents))
                )
            );
    }
